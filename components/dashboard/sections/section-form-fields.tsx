...
import { TONE_OPTIONS } from "@/constant";

interface SectionFormFieldsProps {
  formData: FormDataSection;
  setFormData: (data: FormDataSection) => void;
  selectedSources: string[];
  setSelectedSources: (sources: string[]) => void;
  knowledgeSources: SectionKnowledgeSource[];
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <Select
            ...
            disabled={isDisabled}
          >
            <SelectTrigger className="w-full bg-white/2 border-white/10 text-white">
              <SelectValue
                placeholder={
                  isLoadingSources
                    ? "Loading sources..."
              ...
              >
                {knowledgeSources.find((s) => s.id === selectedSources[0])
                  ?.name || null}
              </SelectValue>
            </SelectTrigger>

            <SelectContent className=" bg-[#0A0A0E] border border-white/10 text-zinc-300">
              <SelectGroup>
              ...
              >
                ...
                <Label htmlFor={option.value} className="flex-1 cursor-pointer">
                  <div className="flex items-center gap-2">
                    ...
                    {option.badge && (
                      <span className="text-[10px] bg-emerald-500/10 text-emerald-500 px-1.5 py-1 rounded-sm border border-emerald-500/10">
                        {option.badge}
                      </span>
                    )}
                  </div>
